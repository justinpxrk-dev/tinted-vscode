...
async function addThemesToPackageJson(system) {
  ...
  const packageJsonThemeObjects = fs
    .readdirSync(systemThemesPath)
    .map((filename) => {
      const themeJson = parse(
        fs.readFileSync(path.join(systemThemesPath, filename), "utf8"),
      );

      return {
        label: filename.replace(/(.+)\.json/, "$1"),
        uiTheme: themeJson.type === "light" ? "vs" : "vs-dark",
...
async function main() {
  await removeThemesfromPackageJson();
